...
import { colorChoices, horizontalHoldPadding, verticalHoldPadding} from "./config.js";
...
export class HoldManager {
    ...
    randomHoldValues(numHolds, index) {
        ...
        const minX = this.canvas.width * horizontalHoldPadding;
        const maxX = this.canvas.width * (0.5 - horizontalHoldPadding / 2);
        const x = Math.random() * (maxX - minX) + minX; // need it between minX and maxX

        const minYOverall = this.canvas.height * verticalHoldPadding; // The minimum possible y position for any hold
